...
class AlignmentLayer(nn.Module):
    ...
    5. Explicit device handling to prevent tensor mismatches.
    """
    ...
    def __init__(
        ...
        Args:
            sym_dim: Dimension of symbolic embeddings (e.g., 300 for typical word embeddings).
            neural_dim: Dimension of neural embeddings (e.g., 768 for BERT-based models).
    ...
    def dynamic_project(
        ...
        """
        try:
            device = sym_emb.device  # Ensure consistency with symbolic embedding device

            # Ensure projections are performed on the correct device
            sym_emb = sym_emb.to(device)
            ...
            sym_heads = sym_projected.view(batch_size, -1, self.num_heads, self.head_dim).to(device)
            neural_heads = neural_projected.view(batch_size, -1, self.num_heads, self.head_dim).to(device)

            # Calculate attention scores
            attention_scores = torch.matmul(sym_heads, neural_heads.transpose(-2, -1)) / self.scale
            attention_weights = F.softmax(attention_scores, dim=-1).to(device)

            # Apply attention
            attended_features = torch.matmul(attention_weights, neural_heads).to(device)
    ...
    def forward(
        ...
        try:
            ...
            if sym_emb.device != neural_emb.device:
                logger.info(f"Moving sym_emb from {sym_emb.device} to {neural_emb.device}")
            ...
            debug_info = {
                ...
                'confidence_score': confidence_score.detach().cpu().item(),
